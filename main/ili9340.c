...
void spi_master_init(TFT_t *dev, int16_t GPIO_MISO, int16_t GPIO_MOSI,
                     int16_t GPIO_SCLK, int16_t GPIO_CS, int16_t GPIO_DC,
                     int16_t GPIO_RESET, int16_t GPIO_BL)
{
  ...
  ESP_LOGI(TAG, "spi_bus_initialize: %s at DMA channel: %d", ret == ESP_OK ? "OK" : "FAILED", DMA_CHAN);
  assert(ret == ESP_OK);
  ...
  ESP_LOGI(TAG, "TFT spi_bus_add_device: %s freq: %d", ret == ESP_OK ? "OK" : "FAILED", SPI_Frequency);
  assert(ret == ESP_OK);
  dev->_dc = GPIO_DC;
  dev->_bl = GPIO_BL;
  ...
  // XPT2046 IRQ config
  gpio_config_t touch_irq_config = {
      .pin_bit_mask = BIT64(CONFIG_TOUCH_IRQ_GPIO),
  ...
  ret = gpio_config(&touch_irq_config);
  assert(ret == ESP_OK);
  ESP_LOGI(TAG, "XPT2046 IRQ initialized");

  // XPT2046 CS
  ESP_LOGI(TAG, "GPIO_TOUCH_CS=%d", CONFIG_TOUCH_CS_GPIO);

  gpio_config_t touch_cs_io_conf = {
      .pin_bit_mask = BIT64(CONFIG_TOUCH_CS_GPIO),
  ...
  ret = gpio_config(&touch_cs_io_conf);
  assert(ret == ESP_OK);

  gpio_set_level(CONFIG_TOUCH_CS_GPIO, 1);
  ESP_LOGI(TAG, "XPT2046 CS initialized");

  spi_device_interface_config_t touch_devcfg = {
      .clock_speed_hz = (2 * 1000 * 1000),
      .spics_io_num = CONFIG_TOUCH_CS_GPIO,
      .queue_size = 1,
      .pre_cb = NULL,
      .post_cb = NULL,
      .command_bits = 8,
  ...
  ret = spi_bus_add_device(LCD_HOST, &touch_devcfg, &touch_handle);
  ESP_LOGI(TAG, "XPT2046 spi_bus_add_device: %s", ret == ESP_OK ? "OK" : "FAILED");
  assert(ret == ESP_OK);
  dev->_TouchHandle = touch_handle;
}
...
void lcdInit(TFT_t *dev, uint16_t model, int width, int height, int offsetx,
             int offsety)
{
  dev->_model = model;
  dev->_width = width;
  ...
  if (dev->_model == 0x7796)
  {
    ...
    spi_master_write_data_byte(dev,
                               0x48); // Right top start, BGR color filter panel
    // spi_master_write_data_byte(dev, 0x68);	//Right top start, BGR color
    // filter panel
  ...
  if (dev->_model == 0x9340 || dev->_model == 0x9341 || dev->_model == 0x7735)
  {
    if (dev->_model == 0x9340)
      ESP_LOGI(TAG, "TFT is ILI9340");
    if (dev->_model == 0x9341)
      ESP_LOGI(TAG, "TFT is ILI9341");
    if (dev->_model == 0x7735)
      ESP_LOGI(TAG, "TFT is ST7735");
    ESP_LOGI(TAG, "Screen width:%d", width);
    ESP_LOGI(TAG, "Screen height:%d", height);
    spi_master_write_comm_byte(dev, 0xC0); // Power Control 1
    ...
    spi_master_write_comm_byte(dev, 0x36); // Memory Access Control
    spi_master_write_data_byte(dev,
                               0x08); // Right top start, BGR color filter panel
    // spi_master_write_data_byte(dev, 0x00);//Right top start, RGB color filter
    // panel
  ...
  if (dev->_model == 0x9226)
  {
    ...
    lcdWriteRegisterByte(dev, 0x0C,
                         0x0000);            // RGB interface setting R0Ch=0x0110 for RGB
    ...
    lcdWriteRegisterByte(dev, 0x11,
                         0x1F3F); // Set APON,PON,AON,VCI1EN,VC----1038
    delayMS(50);
    lcdWriteRegisterByte(dev, 0x12,
                         0x0121);            // Internal reference voltage= Vci;----1121
...
void lcdBGRFilter(TFT_t *dev)
{
      ...
      dev->_model == 0x7796)
  {
    spi_master_write_comm_byte(dev, 0x36); // Memory Access Control
    spi_master_write_data_byte(dev,
                               0x00); // Right top start, RGB color filter panel
  }                                   // endif 0x9340/0x9341/0x7735/0x7796

  if (dev->_model == 0x9225 || dev->_model == 0x9226)
  {
    lcdWriteRegisterByte(dev, 0x03,
                         0x0030); // set GRAM write direction and BGR=0.
  }                               // endif 0x9225/0x9226
}
// Fill screen
// color:color
...
// ascii: ascii code
// color:color
int lcdDrawChar(TFT_t *dev, FontxFile *fxs, uint16_t x, uint16_t y,
  ...
  for (h = 0; h < ph; h++)
  {
    ...
    for (w = 0; w < ((pw + 4) / 8); w++)
    {
      ...
      for (bit = 0; bit < 8; bit++)
      {
        ...
        {
          // if (dev->_font_fill) lcdDrawPixel(dev, xx, yy,
          // dev->_font_fill_color);
        }
...
// UnSet font filling
void lcdUnsetFontFill(TFT_t *dev) { dev->_font_fill = false; }
...
void lcdUnsetFontUnderLine(TFT_t *dev) { dev->_font_underline = false; }
...
void lcdSetScrollArea(TFT_t *dev, uint16_t tfa, uint16_t vsa, uint16_t bfa)
{
  ...
  {
    lcdWriteRegisterByte(
        dev, 0x31, vsa); // Specify scroll end and step at the scroll display
    lcdWriteRegisterByte(
        dev, 0x32, tfa); // Specify scroll start and step at the scroll display
#if 0
...
void lcdResetScrollArea(TFT_t *dev, uint16_t vsa)
{
  ...
  {
    lcdWriteRegisterByte(
        dev, 0x31, 0x0); // Specify scroll end and step at the scroll display
    lcdWriteRegisterByte(
        dev, 0x32, 0x0); // Specify scroll start and step at the scroll display
    // lcdWriteRegisterByte(dev, 0x31, vsa);	// Specify scroll end and step
    // at the scroll display lcdWriteRegisterByte(dev, 0x32, tfa);	//
    // Specify scroll start and step at the scroll display
  } // endif 0x9225/0x9226
}
...
static IRAM_ATTR void tp_spi_rw(TFT_t *dev, uint8_t xpt2046_control_byte, uint8_t *out_data)
{
  
  spi_transaction_t transaction = {
      .flags = 0,
      .length = 24,                // total length 1 byte request + 2bytes return data
      .cmd = xpt2046_control_byte, // 1byte request
      .rx_buffer = out_data,       //
      .rxlength = 16,              // 2bytes return data
  };

  // Read - send first byte as command
  esp_err_t ret = spi_device_transmit(dev->_TouchHandle, &transaction);
  assert(ret == ESP_OK);
}
...
static void xpt2046_corr(int16_t *x, int16_t *y)
{
  ...
  *y = swap_tmp;
  ...
  (*x) = (int32_t)((uint32_t)(*x) * TFT_HOR_RES) /
         (XPT2046_X_MAX - XPT2046_X_MIN);

  (*y) = (int32_t)((uint32_t)(*y) * TFT_VER_RES) /
...
static bool IRAM_ATTR xpt2046_is_touched()
{
  return gpio_get_level(CONFIG_TOUCH_IRQ_GPIO) == 0;
}

bool xpt2046_read(TFT_t *dev, int16_t *x_out, int16_t *y_out)
{
  ...
  {
    tp_spi_rw(dev, TOUCH_CMD_X_READ, (uint8_t *)&data);
    x = (data[0] << 8) | data[1];

    tp_spi_rw(dev, TOUCH_CMD_Y_READ, (uint8_t *)&data);
    y = (data[0] << 8) | data[1];

    // ESP_LOGI(TAG, "(%x, %x) y: " BYTE_TO_BINARY_PATTERN " " BYTE_TO_BINARY_PATTERN,
    //          x, y,
...
#if 0
static int tp_get_data_xpt2046(TFT_t *dev, uint8_t type, int samples)
{
  if (dev == NULL)
    return 0;
  ...
  // one dummy read
  tp_spi_rw(dev, type, (uint8_t *)&result, 2);
  ...
  while (i < 10)
  {
    ...
    for (n = 0; n < samples; n++)
    {
      tp_spi_read_reg(dev, type, (uint8_t *)&result, 2);
      if (result < 0)
        break;
