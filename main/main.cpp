...
static const char *TAG = "WroverTFT";

static void list_directory(char *path, bool show_all_files)
{
  ...
  while (true)
  {
    ...
    if (entry->d_type == DT_DIR) // folder
    {
      ...
      size_t buffer_size = strlen(path) + name_length + 2;
      char *new_path = (char *)calloc(buffer_size, sizeof(char));

      assert(new_path != NULL);

      snprintf(new_path, buffer_size, "%s/%s", path, entry->d_name);

      ESP_LOGI(TAG, "Scanning: \"%s\"", new_path);
      list_directory(new_path, show_all_files);
    }
    else // file
    {
      if (!show_all_files)
      {
        if (name_length > 4 && strstr(entry->d_name, ".mp3"))
        {
...
TickType_t ArrowTest(TFT_t *dev, FontxFile *fx, uint16_t model, int width,
                     int height)
{
  TickType_t startTick, endTick, diffTick;
  startTick = xTaskGetTickCount();
  ...
  if (model == 0x9225)
    strcpy((char *)ascii, "ILI9225");
  if (model == 0x9226)
    strcpy((char *)ascii, "ILI9225G");
  if (model == 0x9340)
    strcpy((char *)ascii, "ILI9340");
  if (model == 0x9341)
    strcpy((char *)ascii, "ILI9341");
  if (model == 0x7735)
    strcpy((char *)ascii, "ST7735");
  if (model == 0x7796)
...
TickType_t DirectionTest(TFT_t *dev, FontxFile *fx, int width, int height)
{
  ...
  lcdDrawString(dev, fx, (width - 1), (height - 1) - (fontHeight * 1), ascii,
                color);
...
TickType_t HorizontalTest(TFT_t *dev, FontxFile *fx, int width, int height)
{
  ...
  strcpy((char *)ascii, "Direction=2");
  lcdSetFontDirection(dev, 2);
  lcdDrawString(dev, fx, width - 1, height - (fontHeight * 1) - 1, ascii,
                color);
  lcdSetFontUnderLine(dev, BLUE);
  lcdDrawString(dev, fx, width - 1, height - (fontHeight * 2) - 1, ascii,
                color);
  lcdUnsetFontUnderLine(dev);

  lcdSetFontFill(dev, YELLOW);
  lcdDrawString(dev, fx, width - 1, height - (fontHeight * 3) - 1, ascii,
                color);
  lcdSetFontUnderLine(dev, BLUE);
  lcdDrawString(dev, fx, width - 1, height - (fontHeight * 4) - 1, ascii,
                color);
  lcdUnsetFontFill(dev);
  lcdUnsetFontUnderLine(dev);
...
TickType_t ScrollTest(TFT_t *dev, FontxFile *fx, int width, int height)
{
  ...
  ESP_LOGD(__FUNCTION__, "height=%d fontHeight=%d lines=%d", height, fontHeight,
           lines);
...
TickType_t BMPTest(TFT_t *dev, char *file, int width, int height)
{
  ...
  assert(ret == 2);
  ESP_LOGD(__FUNCTION__, "result->header.magic=%c %c", result->header.magic[0],
           result->header.magic[1]);
  ...
  if ((result->dib.depth == 24) && (result->dib.compress_type == 0))
  {
    ...
    ESP_LOGD(__FUNCTION__, "_x=%d _w=%d _cols=%d _cole=%d", _x, _w, _cols,
             _cole);
...
TickType_t JPEGTest(TFT_t *dev, char *file, int width, int height)
{
  ...
  esp_err_t err =
      decode_image(&pixels, file, _width, _height, &imageWidth, &imageHeight);
  if (err == ESP_OK)
  {
    ESP_LOGI(__FUNCTION__, "imageWidth=%d imageHeight=%d", imageWidth,
             imageHeight);
  ...
  {
    ESP_LOGE(__FUNCTION__, "decode_image err=%d imageWidth=%d imageHeight=%d",
...
void png_init(pngle_t *pngle, uint32_t w, uint32_t h)
{
  ...
  ESP_LOGD(__FUNCTION__, "screenWidth=%d screenHeight=%d", pngle->screenWidth,
           pngle->screenHeight);
  pngle->imageWidth = w;
  pngle->imageHeight = h;
  ...
  // Calculate Reduction
  if (pngle->screenWidth < pngle->imageWidth ||
      pngle->screenHeight < pngle->imageHeight)
  {
    pngle->reduction = true;
    double factorWidth = (double)pngle->screenWidth / (double)pngle->imageWidth;
    double factorHeight =
        (double)pngle->screenHeight / (double)pngle->imageHeight;
  ...
  ESP_LOGD(__FUNCTION__, "reduction=%d scale_factor=%f", pngle->reduction,
           pngle->scale_factor);
  ESP_LOGD(__FUNCTION__, "imageWidth=%d imageHeight=%d", pngle->imageWidth,
           pngle->imageHeight);
}

void png_draw(pngle_t *pngle, uint32_t x, uint32_t y, uint32_t w, uint32_t h,
              uint8_t rgba[4])
{
...
#endif
  lcdInit(&dev, model, CONFIG_WIDTH, CONFIG_HEIGHT, CONFIG_OFFSETX,
          CONFIG_OFFSETY);
